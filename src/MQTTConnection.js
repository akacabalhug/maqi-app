...
export default class MQTTConnection {
  ...
  send(channel = null, payload) {
    ...
    if (!this.mqtt || !this.mqtt.isConnected()) {
      console.log("here1", this.mqtt, this.mqtt.isConnected());
      return;
    }
    ...
    console.log(
      `MQTTConnection send publish channel: ${channel}, payload: ${payload} qos: ${this.QOS} retained: ${this.RETAIN}`
    );
